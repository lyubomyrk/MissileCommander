...
int main()
{
	...
	while (!WindowShouldClose()) {
		...
		for (auto& missile : missiles) {
			...
			if (missile->disassembly) {
				missile->explosion_radius = missile->explosion_radius + missile->radius_dir * missile->growth_speed * dt;
		...
		for (auto& missile : missiles) {
			if (missile->disassembly) {
				DrawCircle(missile->pos.x, missile->pos.y, missile->explosion_radius, WHITE);
				missile->vel = { 0., 0, };
				continue;
			}
